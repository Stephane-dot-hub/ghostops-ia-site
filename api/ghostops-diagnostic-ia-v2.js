// /api/ghostops-diagnostic-ia-v2.js
...
export default async function handler(req, res) {
  ...
  // 3) Récupération robuste du body
  let payload = {};

  try {
    if (req.body && typeof req.body === 'object') {
      // Cas standard : body déjà parsé par le framework
      payload = req.body;
    } else if (typeof req.body === 'string') {
      // Body brut en string JSON
      payload = req.body ? JSON.parse(req.body) : {};
    } else {
      // Fallback : lecture brute du flux
    ...
    console.error('Erreur de parsing du body Diagnostic IA v2 :', e, req.body);
    payload = {};
  }

  console.log('Payload Diagnostic IA v2 reçu :', payload);
  ...
  if (typeof description === 'string' && description.trim().length > 0) {
    ...
    // Fallback : on encapsule le JSON brut pour ne pas renvoyer une 400 stérile
  ...
  if (!effectiveDescription) {
    // Ici seulement si vraiment rien n’est reçu
    return res.status(400).json({
      error:
        'Le serveur ne reçoit aucun contenu exploitable pour le Diagnostic IA (body vide).',
    });
  }
...
- Ne modifie pas les titres des sections.
  ...
  try {
    ...
    if (!openaiResponse.ok) {
      console.error('Erreur OpenAI Diagnostic IA v2:', data);
    ...
    console.error('Erreur lors de l’appel à OpenAI (Diagnostic IA v2):', err);
    return res.status(500).json({
      error:
